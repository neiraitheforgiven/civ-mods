...
local function JFD_Epithets_TeamSetEra(teamID, eraID)
	...
	if epithetProgress > epithetThreshold then
		...
		local epithetID = player:GetBestEpithet()
		if epithetID ~= -1 then 
			...
			if epithet.ConvertsNonFollowers then
				...
				for city in player:Cities() do
					...
					local numReligions = #g_Religions_Table
					for index = 1, #g_Religions_Table do
			...
			for index = 1, numEpithets do
				...
				player:SetEpithetProgress(epithetID, 0)
				player:SetTotalEpithetProgress(0)	
			end
		end
...
-- EPITHETS
----------------------------------------------------------------------------------------------------------------------------
--JFD_Epithets_BarbariansCampCleared
...
local function JFD_Epithets_UnitCreated(playerID, unitID, unitTypeID)
	...
	if unit:IsGreatPerson() then
		local unitClassType = GameInfo.UnitClasses[unit:GetUnitClassType()].Type
