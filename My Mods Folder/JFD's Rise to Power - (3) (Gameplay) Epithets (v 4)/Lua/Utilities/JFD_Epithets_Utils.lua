...
--g_Trait_JFD_EpithetMods_Table
local g_Trait_JFD_EpithetMods_Table = {}
local g_Trait_JFD_EpithetMods_Count = 1
for row in DB.Query("SELECT * FROM Trait_JFD_EpithetMods;") do 	
	g_Trait_JFD_EpithetMods_Table[g_Trait_JFD_EpithetMods_Count] = row
	g_Trait_JFD_EpithetMods_Count = g_Trait_JFD_EpithetMods_Count + 1
end
--==========================================================================================================================
-- GAME DEFINES
--==========================================================================================================================
...
function Player.GetTotalEpithetProgress(player)
	...
	elseif GameInfo then
		local epithetThreshold = GameInfo.Eras[previousEraID].EpithetProgressThreshold
		local currentEpithetThreshold = GameInfo.Eras[currentEraID].EpithetProgressThreshold
		local epithetProgress = g_GetRandom(1,epithetThreshold+2)
...
function Player.ChangeTotalEpithetProgress(player, changeVal)	
	...
	--g_Trait_JFD_EpithetMods_Table
	local traitsTable = g_Trait_JFD_EpithetMods_Table
	...
	if player:IsGoldenAge() then
		...
		for index = 1, numTraits do
			...
			local traitID = GameInfoTypes[row.TraitType]
			if player:HasTrait(traitID) then
				numEpithetProgressMod = numEpithetProgressMod + row.EpithetProgressModifierDuringGA
			end
		end
	end
	...
	player:SetTotalEpithetProgress(newVal)
...
----------------------------------------------------------------------------------------------------------------------------
--Player:SetTotalEpithetProgress
function Player.SetTotalEpithetProgress(player, setVal)	
	local playerID = player:GetID()
...
function Player.ChangeEpithetProgress(player, epithetID, changeVal)	
	...
	if player:IsHuman() then
		...
		player:SetEpithetProgress(epithetID, newVal)
	end
	player:ChangeTotalEpithetProgress(changeVal)
end
----------------------------------------------------------------------------------------------------------------------------
--Player:SetEpithetProgress
function Player.SetEpithetProgress(player, epithetID, setVal)	
	local playerID = player:GetID()
	local epithet = GameInfo.JFD_Epithets[epithetID]
...
function Player.GetBestEpithet(player)	
	local playerID = player:GetID()
