...
function UpdateUnitHealthBar(unit)
	...
	if damage == 0 then
		...
		Controls.HealthBar:SetToolTipString( Locale.ConvertTextKey( "TXT_KEY_UPANEL_SET_HITPOINTS_TT",(unit:GetMaxHitPoints()-damage), unit:GetMaxHitPoints() ) );
	...
	if (bBuild) then
		...
		do
			...
			if (iYieldChange ~= 0) then
				...
				elseif (iYield == GameInfoTypes.YIELD_GREAT_GENERAL_POINTS) then
					strBuildYieldString = strBuildYieldString .. Locale.ConvertTextKey("TXT_KEY_BUILD_GGP_STRING", iYieldChange);
				elseif (iYield == GameInfoTypes.YIELD_GREAT_ADMIRAL_POINTS) then
