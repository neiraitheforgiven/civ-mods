...
local function updateSmallUnitIcons( unit, instance )
	...
	if damage > 0 then
		if instance == Controls then
			percent = 1 - damage / unit:GetMaxHitPoints() / 3
		else
			percent = 1 - damage / unit:GetMaxHitPoints()
		end
...
-- TODO				local numResourceAvailable = g_activePlayer:GetNumResourceAvailable(resource.ID, true)
		...
		-- Health
		local damage = unit:GetDamage()
		tips:insertIf( damage > 0 and L( "TXT_KEY_UPANEL_SET_HITPOINTS_TT", unit:GetMaxHitPoints()-damage, unit:GetMaxHitPoints() ) )
...
local function UpdateUnitPortrait( unit )
	...
	local name
	if (unit:GetUnitClassType() == GameInfoTypes.UNITCLASS_GREAT_GENERAL or unit:GetUnitClassType() == GameInfoTypes.UNITCLASS_GREAT_ADMIRAL) and unit:IsHasPromotion(GameInfoTypes.PROMOTION_NOTHING) then
		name = unit:GetNameNoDesc()
		if(name == nil or #name == 0) then
			name = unit:GetName();			
		end
	elseif (unit:GetUnitClassType() == GameInfoTypes.UNITCLASS_GREAT_GENERAL or unit:GetUnitClassType() == GameInfoTypes.UNITCLASS_GREAT_ADMIRAL) and unit:GetExperience() >= Expneeded2 then
		name = (unit:GetName() .. " (" .. unit:GetExperience() .. "/" .. Expneeded3 .. ")")
	elseif (unit:GetUnitClassType() == GameInfoTypes.UNITCLASS_GREAT_GENERAL or unit:GetUnitClassType() == GameInfoTypes.UNITCLASS_GREAT_ADMIRAL) and unit:GetExperience() < Expneeded2 then
		name = (unit:GetName() .. " (" .. unit:GetExperience() .. "/" .. Expneeded2 .. ")")
...
local function UpdateUnitHealthBar(unit)
	...
	if damage == 0 then
		...
		local healthPercent = 1.0 - (damage / unit:GetMaxHitPoints())
		...
		Controls.HealthBar:LocalizeAndSetToolTip( "TXT_KEY_UPANEL_SET_HITPOINTS_TT", unit:GetMaxHitPoints()-damage, unit:GetMaxHitPoints() )
