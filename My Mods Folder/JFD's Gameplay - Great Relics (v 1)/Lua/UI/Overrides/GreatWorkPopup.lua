...
function ShowHideHandler( bIsHide, bInitState )
    if( not bInitState ) then
        if( not bIsHide ) then
			...
			Controls.GreatWorkSplash:SetAnchor("L,T")				
			if greatWork.GreatWorkClassType == "GREAT_WORK_RELIC" then
				Controls.TopIcon:SetTexture("JFD_GreatRelics_IconAtlas_128.dds");
				m_fOriginalSizeX = m_fOriginalSizeX + Controls.Title:GetSizeX();
				m_fOriginalSizeY = m_fOriginalSizeY + Controls.Title:GetSizeY();
				Controls.GreatWorkSplash:SetAnchor("C,T")
				Controls.GreatWorkSplash:ReprocessAnchoring()
			end
