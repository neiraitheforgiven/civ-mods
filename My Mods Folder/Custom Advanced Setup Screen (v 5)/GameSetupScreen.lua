...
print("Loading Custom Quick Setup Screen...")
print("-------------------------------------")
...
function OnStart()
	...
	if bRevolutionMod then -- set number of CS if Revolution mod is loaded
		print("Launching game (Revolution mod is active)...")
		print("-------------------------------------")
		local info = GameInfo.Worlds[ PreGame.GetWorldSize() ]
		local numMinor = 12 -- arbitrary default
		if ( info ~= nil ) then
			numMinor = info.DefaultMinorCivs
		end
		print("Num minor civ for this map/size = " .. numMinor)
		modUserData.SetValue ("NumMinorCivs", numMinor)
		PreGame.SetNumMinorCivs(#GameInfo.MinorCivilizations)
	else
		print("Launching game (Revolution mod is not active)...")
		print("-------------------------------------")
	end
