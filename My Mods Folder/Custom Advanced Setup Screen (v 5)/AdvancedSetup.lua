...
print("Loading Custom Advanced Setup Screen...")
print("-------------------------------------")
...
local RevolutionModID = "cbda59cb-f254-41ad-8d69-ea5053e048f8"
local bDynHist = false
local bRevolutionMod = false
...
-------------------------------------------------
local DynHistModVersion = Modding.GetLatestInstalledModVersion(DynHistModID) -- use DynHist mod ID to allow data sharing between component required by that mod.
local modUserData = Modding.OpenUserData(DynHistModID, DynHistModVersion)
	...
	for _, option in ipairs(sortedOptions) do
		...
		for _, possibleValue in ipairs(option.Values) do
			...
			controlTable.Button:RegisterCallback(Mouse.eLClick, function()
				...
				modUserData.SetValue ("MapOption_"..option.ID, possibleValue.Value)
			end);
		end
		...
		if (savedValue == nil) then
			...
			PreGame.SetMapOption(option.ID, savedValue)
		end
		local defaultValue;
...
["MapSizes"] = {
	FullSync = function()
		...
		pullDown:RegisterSelectionCallback( function(id)
			if( id == -1 ) then
				...
				modUserData.SetValue ("WorldSize", id)
				modUserData.SetValue ("NumMinorCivs", mapSize.DefaultMinorCivs)
	...
	PartialSync = function()
		...
		local bIsMapScript = PreGame.IsRandomMapScript();
		...
		Controls.MapSizePullDown:GetButton():SetDisabled(not bIsMapScript and not PreGame.IsEarthMap());
...
["MapTypes"] = {
	FullSync = function()
		...
		local mapScripts = {
			-- Earth Entry
			{
				Name = Locale.ConvertTextKey("TXT_KEY_MAP_EARTH_TITLE"),
				Description = Locale.ConvertTextKey("TXT_KEY_MAP_EARTH_HELP"),
				IsEarthMap = true,
			},
		};
		...
		for row in GameInfo.MapScripts{SupportsSinglePlayer = 1} do
			local script = {};
		...
		pullDown:RegisterSelectionCallback( function(id)
			...
			elseif(mapScript.IsEarthMap) then
				...
				modUserData.SetValue ("MapScript", mapScript.FileName)
				
				if(mapScript.DefaultCityStates ~= nil) then
					--PreGame.SetNumMinorCivs(mapScript.DefaultCityStates);
					modUserData.SetValue ("NumMinorCivs", mapScript.DefaultCityStates)
				end
				...
				if(mapScript.WBMapData ~= nil) then
					...
					modUserData.SetValue ("NumMinorCivs", wb.CityStateCount)
					PreGame.SetRandomWorldSize(false);
	...
	PartialSync = function()
		...
		if( not PreGame.IsRandomMapScript() ) then 
			...
			if(PreGame.IsEarthMap()) then
				pullDown:GetButton():LocalizeAndSetText("TXT_KEY_MAP_EARTH_TITLE");
...
["MinorCivs"] = {
	FullSync = function()
		function OnMinorCivsSlider( fValue )
			modUserData.SetValue ("NumMinorCivs", math.floor( fValue * #GameInfo.MinorCivilizations ) )
			--PreGame.SetNumMinorCivs( math.floor( fValue * #GameInfo.MinorCivilizations ) );
			Controls.MinorCivsSlider:SetValue( modUserData.GetValue("NumMinorCivs") / #GameInfo.MinorCivilizations );
			Controls.MinorCivsLabel:LocalizeAndSetText("TXT_KEY_AD_SETUP_CITY_STATES", math.floor(fValue * #GameInfo.MinorCivilizations));
		end
		Controls.MinorCivsSlider:RegisterSliderCallback( OnMinorCivsSlider );
	end,
	
	PartialSync = function()
		local numMinor = modUserData.GetValue("NumMinorCivs") or 12 
		if ( not PreGame.IsRandomWorldSize() ) then
			
			Controls.MinorCivsSlider:SetHide(false);
			Controls.MinorCivsLabel:SetHide(false);

			Controls.MinorCivsSlider:SetValue( numMinor / #GameInfo.MinorCivilizations );
			Controls.MinorCivsLabel:LocalizeAndSetText("TXT_KEY_AD_SETUP_CITY_STATES", numMinor);
		else
...
function OnStartClicked()
	...
	if bRevolutionMod then -- set number of CS if Revolution mod is loaded
		print("Launching ".. gameText .." (Revolution mod is active)...")
		print("-------------------------------------")
		PreGame.SetNumMinorCivs(#GameInfo.MinorCivilizations)
	else
		print("Launching ".. gameText .." (Revolution mod is not active)...")
		print("-------------------------------------")
		local numMinor = modUserData.GetValue("NumMinorCivs") or 12 -- yeah, that's arbitrary
		PreGame.SetNumMinorCivs(numMinor)
	end
	print(" - num civs = " .. modUserData.GetValue ("NumPlayers") )
	print(" - num minors = " .. modUserData.GetValue("NumMinorCivs") )
...
function OnDefaultsClicked()
	...
	modUserData.SetValue ("NumMinorCivs", worldSize.DefaultMinorCivs)
	...
	local handicap = GameInfo.HandicapInfos["HANDICAP_CHIEFTAIN"];
...
function RevolutionSetup ()
	...
	-- Map Size
	local worldSizeID = modUserData.GetValue("WorldSize")
	if (worldSizeID) == nil then
		local worldSize = GameInfo.Worlds["WORLDSIZE_SMALL"];
		if(worldSize == nil) then
			worldSize = GameInfo.Worlds()(); -- Get first world size found.
		end
		worldSizeID = worldSize.ID
	end
	PreGame.SetRandomWorldSize( false );
	PreGame.SetWorldSize( worldSizeID );

	-- minor civs set to max available, we'll sort them later
	-- but we'll store the user wanted number in modUserData
	--PreGame.SetNumMinorCivs(#GameInfo.MinorCivilizations)
	local numMinor = modUserData.GetValue("NumMinorCivs") or 12 -- yeah, that's arbitrary
	modUserData.SetValue ("NumMinorCivs", numMinor)
	
	-- Map Type
	PreGame.SetLoadWBScenario(false);
	PreGame.SetEarthMap(false);

	local mapFileName = modUserData.GetValue("MapScript")
	if (mapFileName == nil) then
		mapFileName = "Assets\\Maps\\Continents.lua"
	end
	local mapScript = GameInfo.MapScripts{FileName = mapFileName}();
	if(mapScript ~= nil) then
		PreGame.SetRandomMapScript(false);
		PreGame.SetMapScript(mapScript.FileName);
	else
	...
	if numPlayers and numPlayers > 1 then
