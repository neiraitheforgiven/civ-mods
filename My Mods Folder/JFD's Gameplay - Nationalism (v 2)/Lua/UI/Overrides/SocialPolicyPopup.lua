...
function PolicySelected( policyIndex )
	...
	--JFD NATIONALISM
	if GameInfo.Policies[policyIndex].IsNationalism then
		bCanAdoptPolicy = true
    end
...
-------------------------------------------------------------------------------------------------------------------------
--UI_UpdateNationalismPolicyInfo
function UI_UpdateNationalismPolicyInfo(player, policyInfo, thisPolicyIcon, strTT)
	local policyID = policyInfo.ID
	local costNationalism = player:GetNationalismPolicyCost(policyID)
	local numNationalism = player:GetNationalism()
	if player:HasNationalismPolicy(policyID) then
		strTT = strTT .. "[NEWLINE][NEWLINE]" .. Locale.ConvertTextKey("TXT_KEY_JFD_POLICY_NATIONALISM_ADOPTED_TOOLTIP")
		thisPolicyIcon.PolicyIcon:SetDisabled(true)
		thisPolicyIcon.PolicyIcon:SetToolTipString(strTT)
		IconHookup( policyInfo.PortraitIndex, 64, policyInfo.IconAtlasAchieved .. "_NATIONALISM", thisPolicyIcon.PolicyImage );
	elseif numNationalism >= costNationalism then
		thisPolicyIcon.PolicyIcon:SetDisabled(false)
	
		strTT = strTT .. "[NEWLINE][NEWLINE]" .. Locale.ConvertTextKey("TXT_KEY_JFD_POLICY_NATIONALISM_DOUBLE_TOOLTIP")
		local nationalismPolicyID = Player_GetNationalismPolicy(player, policyID)
		thisPolicyIcon.PolicyIcon:SetVoid1(nationalismPolicyID)
		thisPolicyIcon.PolicyIcon:RegisterCallback(Mouse.eLClick, PolicySelected)
		thisPolicyIcon.PolicyIcon:SetToolTipString(strTT)
	else
		strTT = strTT .. "[NEWLINE][NEWLINE]" .. Locale.ConvertTextKey("TXT_KEY_JFD_POLICY_NATIONALISM_UNLOCK_TOOLTIP", costNationalism)
		thisPolicyIcon.PolicyIcon:SetDisabled(true)
		thisPolicyIcon.PolicyIcon:SetToolTipString(strTT)
	end
end
...
function UpdateDisplay()
    ...
    if (iDominantBranch ~= -1) then
		...
		-- JFD
		local strText = GameplayUtilities.GetLocalizedLeaderTitle(player);
...
--	if numUnlockedBranches > 0 then
	...
	while policyInfo ~= nil do
		...
		if (iBranch ~= nil and not GameInfo.PolicyBranchTypes[iBranch].PurchaseByLevel) then
			...
			--JFD NATIONALISM
			local isUpdatePolicyForNationalism = false
			...
			if player:HasPolicy( i ) then
				...
				--JFD NATIONALISM
				----------------------------------------
				isUpdatePolicyForNationalism = true
				----------------------------------------
				--JFD NATIONALISM
				----------------------------------------
			...
			--JFD NATIONALISM
			----------------------------------------
			if isUpdatePolicyForNationalism then
				UI_UpdateNationalismPolicyInfo(player, policyInfo, thisPolicyIcon, strTooltip)
			end
			----------------------------------------
			--JFD NATIONALISM
			----------------------------------------	
...
function OnYes( )
	...
	--JFD NATIONALISM
	if GameInfo.Policies[m_gPolicyID].IsNationalism then
		Players[Game.GetActivePlayer()]:SetHasNationalismPolicy(m_gPolicyID, m_gAdoptingPolicy)
	end
	UpdateDisplay()
end
...
--UI_Update_NationalismInfo
function UI_Update_NationalismInfo(value)
	local player = Players[Game.GetActivePlayer()]
	...
	if maxValue == 0 then
		local strNationalismPerTurnTT = Locale.ConvertTextKey("TXT_KEY_JFD_SLIDER_NATIONALISM_TOOLTIP")
		if numExtraNationalismPerTurn > 0 then
			strNationalismPerTurnTT = Locale.ConvertTextKey("TXT_KEY_JFD_SLIDER_NATIONALISM_TOOLTIP_EXTRA_PER_TURN", numExtraNationalismPerTurn)
		end
		Controls.NationalismPercentSlider:SetDisabled(true)
		Controls.NationalismPerTurn:LocalizeAndSetText("TXT_KEY_JFD_SLIDER_NATIONALISM", numNationalismPerTurn+numExtraNationalismPerTurn, numNationalism)
		Controls.NationalismPerTurn:SetToolTipString(strNationalismPerTurnTT)
		Controls.NationalismPercentValue:LocalizeAndSetText("{1_Num}/{2_Num}%", 0, 0)
	else
		Controls.NationalismPercentSlider:SetDisabled(false)
		if (not value) then 
			value = player:GetNationalismPercent() 
		else
			Player_SetNationalismPercent(player, value)
		...
		Controls.NationalismPerTurn:LocalizeAndSetText("TXT_KEY_JFD_SLIDER_NATIONALISM", numNationalismPerTurn+numExtraNationalismPerTurn, numNationalism)
		Controls.NationalismPerTurn:SetToolTipString(strNationalismPerTurnTT)
		Controls.NationalismPercentValue:LocalizeAndSetText("{1_Num}/{2_Num}%", value, maxValue)
	end
	Controls.CulturePerTurnLabel:LocalizeAndSetText("TXT_KEY_CULTURE_PER_TURN_LABEL", (player:GetTotalJONSCulturePerTurn()-numNationalismPerTurn))
    
	LuaEvents.JFD_Nationalism_UI_UpdateTopPanel()
	Controls.InfoStack3:ReprocessAnchoring()
end
Controls.NationalismPercentSlider:RegisterSliderCallback(UI_Update_NationalismInfo)
LuaEvents.UI_Update_NationalismInfo.Add(UI_Update_NationalismInfo)
UI_Update_NationalismInfo()
--==========================================================================================================================
--==========================================================================================================================